...
class LinearWarmUpCosineAnnealingLR(LambdaLR):
    def __init__(self, optimizer, *, peak, final, warm_up_steps, max_steps, init=1e-8, offset=0, epoch_size=0, stable_steps=0, **kwargs):
        assert peak >= final >= init >= 0
        assert max_steps >= warm_up_steps + stable_steps
        self.init = init
        self.peak = peak
        self.final = final
    ...
    def _step_inner(self, steps):
        ...
        if self.stable_steps > 0 and steps < self.warm_up_steps + self.stable_steps:
            return self.peak
        if steps < self.max_steps:
            cos_steps = steps - self.warm_up_steps - self.stable_steps
            cos_max_steps = self.max_steps - self.warm_up_steps - self.stable_steps
